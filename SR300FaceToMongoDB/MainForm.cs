...
namespace DF_FaceTracking.cs
{
    public partial class MainForm : Form
    {
        ...
        private volatile Bitmap m_bitmap;
        ...
        //Bruce 2016/10
        #region Real-time Insert to MongoDB

        private String insertJsonStr = "";

        public string kid_name = "";
        public int kid_stu_id;
        public string kid_gender;
        public string kid_birthday;
        public string kid_group;
        ...
        public void MongoCon_Click(object sender, EventArgs e)
        {

            using (MongoInfo mongoInfo = new MongoInfo())
            {
                mongoInfo.ShowDialog(this);
                //Console.WriteLine("_client : " + _client.Cluster.Description);
                //Console.WriteLine(System.IO.Directory.GetCurrentDirectory());
                if (mongo_collection != "")
                {
                    this.mongo_icon.BackgroundImage = Image.FromFile(@"..\..\..\pics\mongo_on.png", true);
                }
                else {
                    this.mongo_icon.BackgroundImage = Image.FromFile(@"..\..\..\pics\mongo_off.png", true);
                }
            }
            
        }

        private void btn_kidinfo_Click(object sender, EventArgs e)
        {
            using (KidInfo kidInfo = new KidInfo())
            {
                kidInfo.ShowDialog(this);
                string kid_icon = kid_gender + kid_group;
                switch (kid_icon)
                {
                    case "BT":
                        this.pic_kid_icon.BackgroundImage = Image.FromFile(@"..\..\..\pics\kid_icon_boy.png", true);
                        break;
                    case "BS":
                        this.pic_kid_icon.BackgroundImage = Image.FromFile(@"..\..\..\pics\kid_icon_boy_s.png", true);
                        break;
                    case "GT":
                        this.pic_kid_icon.BackgroundImage = Image.FromFile(@"..\..\..\pics\kid_icon_girl.png", true);
                        break;
                    case "GS":
                        this.pic_kid_icon.BackgroundImage = Image.FromFile(@"..\..\..\pics\kid_icon_girl_s.png", true);
                        break;
                    default:
                        this.pic_kid_icon.BackgroundImage = Image.FromFile(@"..\..\..\pics\kid_icon_unknown.png", true);
                        break;
                }
            }
        }

        private void btn_RecordSwitch_Click(object sender, EventArgs e)
        {
            //Check if it is ready to record
            if (mongo_collection == "" || kid_name == "")
            {
                MessageBox.Show("Please check Kid Info and MongoDB connection!");
                return;
            }
            ...
            Start.Enabled = false;
            Stop.Enabled = false;
            btn_kidinfo.Enabled = false;
            MongoCon.Enabled = false;
            btn_RecordSwitch.Enabled = false;
            ...
            ft.initiateRecordThread();
            duration = Int32.Parse(txt_duration.Text) * 1;
            ft.isRecording = true;
            ft.recordData.Start();
        }
        ...
        void timer_Tick(object sender, EventArgs e)
        {
            ...
            {
                duration--;
                this.lbl_timer.Text = "Remained: " + duration / 60 + ":" + duration % 60;

            }
            else
            {
                ft.isRecording = false;
        ...
        public void insertFeaturesToMongoDB(PXCMFaceData.Face face, int framecount, String time)
        {
            insertJsonStr = "";
            insertJsonStr += "{\"count\":"   + framecount + 
                ",\"datetime\":ISODate(\""   + time +
                "\"),\"name\":\""            + kid_name +
                "\",\"stu_id\":\""           + kid_stu_id +
                "\",\"group\":\""            + kid_group +
                "\",\"gender\":\""           + kid_gender +
            ...
            //rectangle: detected face location x,y
            if (Detection.Checked)
            {
                ...
                insertJsonStr += "{\"detected_id\":"+face.QueryUserID();
                //String faceId = String.Format("Face ID: {0}", face.QueryUserID().ToString(CultureInfo.InvariantCulture));
            }

            //landmarks: 78 points on face
            if (Landmarks.Checked)
            {
                insertJsonStr += ",\"landmarks\":[";
                PXCMFaceData.LandmarksData landmarks = face.QueryLandmarks();
                if (m_bitmap == null || !Landmarks.Checked || landmarks == null) return;
                ...
                //{
                    ...
                    foreach (PXCMFaceData.LandmarkPoint landmark in points)
                    {
                        ...
                        //    graphics.DrawString("•", font, brush, point);
                        if(index!=77)
                            insertJsonStr += "{\"lm_id\":" + index++ + ",\"2d\":{\"confidence\":" + landmark.confidenceImage + ",\"x\":" + landmark.image.x + ",\"y\":" + landmark.image.y + "},\"3d\":{\"confidence\":" + landmark.confidenceWorld + ",\"x\":" + landmark.world.x + ",\"y\":" + landmark.world.y + ",\"z\":" + landmark.world.z + "}},";
                        else
                            insertJsonStr += "{\"lm_id\":" + index++ + ",\"2d\":{\"confidence\":" + landmark.confidenceImage + ",\"x\":" + landmark.image.x + ",\"y\":" + landmark.image.y + "},\"3d\":{\"confidence\":" + landmark.confidenceWorld + ",\"x\":" + landmark.world.x + ",\"y\":" + landmark.world.y + ",\"z\":" + landmark.world.z + "}}";
                    }
                //}
            ...
            if (Pose.Checked)
            {
                ...
                //using (var font = new Font(FontFamily.GenericMonospace, m_faceTextOrganizer.FontSize, FontStyle.Bold))
                //{
                    //string yawText = String.Format("Yaw = {0}", Convert.ToInt32(poseAngles.yaw).ToString(CultureInfo.InvariantCulture));
                    //string pitchText = String.Format("Pitch = {0}", Convert.ToInt32(poseAngles.pitch).ToString(CultureInfo.InvariantCulture));
                    //string rollText = String.Format("Roll = {0}", Convert.ToInt32(poseAngles.roll).ToString(CultureInfo.InvariantCulture));
                    insertJsonStr += ",\"pose\":{\"yaw\":" + Convert.ToInt32(poseAngles.yaw) + ",\"pitch\":" + Convert.ToInt32(poseAngles.pitch) + ",\"roll\":" + Convert.ToInt32(poseAngles.roll) + "}";
                //}
            }

            //expression: mouth, eye, eyebrow
            if (Expressions.Checked)
            {
                int index = 0;
                insertJsonStr += ",\"expressions\":[";
                PXCMFaceData.ExpressionsData expressionsOutput = face.QueryExpressions();
                foreach (var expressionEntry in m_expressionDictionary)
                {
                    PXCMFaceData.ExpressionsData.FaceExpression expression = expressionEntry.Key;
                    ...
                    bool status = expressionsOutput.QueryExpression(expression, out result);
                    if (!status) continue;
                    //var expressionText = String.Format("{0} = {1}", expressionEntry.Key, result.intensity);
                    if(index++ != 12)
                        insertJsonStr += "{\"key\":\"" + expressionEntry.Value + "\",\"intensity\":" + result.intensity +"},";
                    else
                        insertJsonStr += "{\"key\":\"" + expressionEntry.Value + "\",\"intensity\":" + result.intensity + "}]";
                }
            }
            ...
            if (Pulse.Checked)
            {
                var pulseData = face.QueryPulse();
                //var pulseString = "Pulse: " + pulseData.QueryHeartRate();
                insertJsonStr += ",\"pulse\":" + pulseData.QueryHeartRate() + "";
            }
            //recognition: face recognition

            //Insert into DB
            insertJsonStr += "}}]}";
            //Console.WriteLine("JsonStr: "+insertJsonStr);
            insert(insertJsonStr);
        }

        public void insertFeaturesToMongoDBStop(){
            Invoke(new Action(()=>{
                Stopped = true;
                MainMenu.Enabled = true;
                Start.Enabled = true;
                Stop.Enabled = true;
                btn_kidinfo.Enabled = true;
                MongoCon.Enabled = true;
                btn_RecordSwitch.Enabled = true;
            }));
        }
